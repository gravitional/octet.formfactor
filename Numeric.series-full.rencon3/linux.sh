...
for lambda in "${lambdaList[@]}"; do
    wolframscript -print 'all' -file $wlscript $order ${lambda}${prcs} '1.00'${prcs} ${fitting}
done

# # start the ci=1.5 numeric
# for lambda in "${lambdaList[@]}"; do
#     wolframscript -print 'all' -file $wlscript $order ${lambda}${prcs} '1.50'${prcs} ${fitting}
# done
...
# for ci in "${ciList[@]}"
# do
# wolframscript -print 'all' -file $wlscript   $order   '0.90'${prcs}  ${ci}${prcs} ${fitting}
# done
